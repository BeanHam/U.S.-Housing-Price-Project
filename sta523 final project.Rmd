...
>>>>>>> 3313579fe28aa3f4bf79389df7e3fdc6306be92a
...
```{r}
rating = function(v){
  mapvalues(v,sort(unique(v)),(1:length(unique(v)))*100/(length(unique(v))))
}
...
num_shoppingmall, college_num, MURDER, EstimatedCountyRate ,public_school_count,CPOPCRIM, ryprop, private_school_count) %>%
  sapply(.,rating) %>% as.data.frame() %>% mutate(
    ...
    mutate(safety = rowMeans(select(.,BURGLRY,ROBBERY,LARCENY,MURDER,CPOPCRIM)),
           ...
           school = rowMeans(select(.,public_school_count,private_school_count,college_num)),
...
county = county_data[,1:2] %>% as.data.frame() 
colnames(county) = c("County","State")
