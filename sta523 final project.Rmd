...
texas = read_wikilist("Texas",50) %>% .[15:50,] %>% separate(location, c("location", "other"), "[(]",remove=TRUE, extra = "merge") %>% 
  select(name, location)
tx_data = rbind(read_texas(1),read_texas(2),read_texas(3),texas) %>% 
  mutate('STATE' = 'TX')
...
shopping_mall_data2 = shopping_mall_data %>%
  select(name, location)%>%
...
shopping_mall_data = bind_cols(shopping_mall_data2,shopping_mall_data) %>% 
