...
read_wikitable_nonflat = function()

read_wikilist = function(state){
url = paste0("https://en.wikipedia.org/wiki/List_of_shopping_malls_in_",state)
page = read_html(url)
dat = data_frame(
...
al_data = data_frame(
  name = read_wikitable("Alabama")[[1]][[1]] %>% as.character(.),
  location=read_wikitable("Alabama")[[1]][[2]] %>% as.character(.)
) %>% mutate('STATE' = 'AL')

n_ca = 181 #known from the wiki
ca_data = read_wikilist("California") %>% .[1:n_ca,] %>%
  separate(name, c("name", "location"), "-",remove=TRUE) %>%
  mutate('STATE' = 'CA') 

n_md = 34 #known from the wiki
md_data = read_wikilist("Maryland") %>% .[1:n_md,] %>%
    separate(name, c("name", "location"), "-",remove=TRUE) %>% 
  mutate('STATE' = 'MD')
...
nj_data = data_frame(
  name = read_wikitable("New_Jersey")[[1]][[1]] %>% as.character(.),
  location=read_wikitable("New_Jersey")[[1]][[2]] %>% as.character(.)
)
...
pa_data = data_frame(
  name = read_wikitable("Pennsylvania")[[1]][[1]] %>% as.character(.),
  location = read_wikitable("Pennsylvania")[[1]][[2]] %>% as.character(.)
) %>% mutate('STATE' = 'PA')
