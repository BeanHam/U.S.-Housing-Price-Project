...
recommend_level = function(x){
  # return an integer in -3 ~ 3
  pmax(0, pmin(3, ceiling(log2((abs(x) + 1e-10) / 0.05)))) * sign(x) + 4
}
recommend_text = c("Highly Highly Not Recommended", "Highly Not Recommended", 
                   "Not Recommended", "Fair Offer", "Recommended", 
                   "Highly Recommended", "Highly Highly Recommended")
county_data = county_data %>%
  mutate(predictions = pred,
         diff = pred - price) %>%
  mutate(recommend = recommend_text[recommend_level(diff)])
### comments ###
# "diff" is a function in R, so it is better to change the name of diff, if possible.
### end ###
```
...
# library(plyr)
### comments ###
# changed mapvalues in func rating to plyr::mapvalues
### end ###
...
save(recommendation_for_shiny, file = "recommendation_part1.RData")
save(county_data_recommend_for_shiny, file = "recommendation_part2.RData")
```
