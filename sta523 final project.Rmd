...
### new ###
price_2017 = read_csv("pricepersqft__.csv") %>%
  select(County,State, `17-Jan`) %>%
  group_by(County,State) %>%
  summarize(price = median(`17-Jan`))
### comments ###
# county_count is never used???
### end ###
```
...
difficult_county = price_2017[price_2017$County %ni% county_data$County,]
### comments ###
# 2 missing values: row 23 and 497, because when downloading from wiki, these two states are District of Columbia, United States
### end ###
```
...
### comments ###
# this part involves serious hardcoding. a solution is to change excel directly. we'll see how to simplify this part after changing excel.
### end ###
```
...
### states = html_text(links) ??? never used???
...
### comments ###
# no need for bind function after simplifying code
### end ###
...
### new ###
tax_rate_data2 = list.files(pattern = "*.csv") %>%
  map(read.csv) %>%
  {do.call(bind_rows, .)} %>%
  ...
  {.[!duplicated(.[,1:2]), ]} %>%
  {left_join(price_2017_5, ., by = c('County','State'))}
### end ###
```
