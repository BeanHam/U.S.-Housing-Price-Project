...
shinyApp(
  ...
  tabPanel("recommendation level", 
             sidebarPanel(
               selectInput("state_rd", "Select A State", choices = state_list, selected = 1),
               actionButton("recommend", "Get recommendation")
               ),
             mainPanel(
               plotOutput("recommendation_level")
               )),

  tabPanel("county rank", 
             sidebarPanel(
               checkboxGroupInput("variable_rd","Select variables to rank", c("Cheap","safety","income","transportation","cpi","economic","tax","school","shopping")),
               actionButton("rank", "Get County Rank"),
               hr(),
               h3("Variable Dictionary: "),
               helpText("- Cheap: "),
               helpText("- Safety: "),
               helpText("- Income: "),
               helpText("- Transportation:"),
               helpText("- CPI: "),
               helpText("- Economic: "),
               helpText("- Tax: "),
               helpText("- School: "),
  ...
  observe({
    ...
    output$recommendation_level = renderPlot({
      barplot=barplotdata()
        ...
        ggplot(data = barplot,aes(x = County, 
                 y = recommend))+geom_bar(stat="identity")+xlab("County") +
        ylab("recommendation level")+theme(axis.text.x = element_text(angle = 40, hjust = 1, vjust = 1)) +coord_flip()
      })
    ...
    tabledata2 = eventReactive(
      input$rank,{
     tabledata2=county_data_recommend_for_shiny %>% select(County,State,input$variable_rd) 
    })
...
test2=c(1:2)
test3=c(3:4)
t=rbind(test1,test2,test3) %>% as.data.frame()
